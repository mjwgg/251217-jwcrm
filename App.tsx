...
const App: React.FC = () => {
  ...
  const handleAddAppointment = useCallback(async (
      ...
      const consultationUpdatedCustomer = await addAppointment(appointment, consultationData, recurrence);
      ...
      if (data.consultationNotes && originalAppointment.customerId) {
        ...
        if (customer) {
            ...
            const updatedCustomer = {
                ...customer,
                consultations: [newConsultation, ...(customer.consultations || [])].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
            };
            await updateCustomer(updatedCustomer);
        }
      }
